  ...
  if (nodes.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[320px] gap-4 p-6">
        <div className="w-14 h-14 rounded-full bg-ink-850/50 flex items-center justify-center">
          <svg className="w-7 h-7 text-fog" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
          </svg>
        </div>
        <div className="text-center max-w-sm">
          <p className="text-haze font-medium mb-1">Sin bigramas disponibles</p>
          <p className="text-fog text-sm leading-relaxed">
            El grafo de co-ocurrencia requiere un análisis de N-gramas con <strong className="text-haze">bigramas</strong> (rango [2,2] o [1,2]).
          </p>
          {availableConfigs.length > 0 && (
            <div className="mt-3 p-3 rounded-lg bg-ink-850/40 border border-ink-700/40 text-left">
              <p className="text-xs text-mist mb-1.5">Configuraciones actuales:</p>
              <div className="flex flex-wrap gap-1.5">
                {availableConfigs.map(c => (
                  <span key={c} className="px-2 py-0.5 rounded-md bg-ink-800/60 text-haze text-xs font-mono">{c}</span>
                ))}
              </div>
              <p className="text-xs text-fog mt-2">→ Para habilitar el grafo, crea un nuevo análisis de N-gramas incluyendo la configuración <span className="font-mono text-mist">[2,2]</span>.</p>
            </div>
          )}
        </div>
      </div>
