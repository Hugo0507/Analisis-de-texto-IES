...
export const WordDetailPanel: React.FC<WordDetailPanelProps> = ({
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-40 flex">
      ...
      <div className="relative ml-auto w-96 bg-ink-900 border-l border-ink-700/50 shadow-2xl flex flex-col h-full overflow-hidden animate-[slideInRight_0.25s_ease-out]">
        <div className="px-5 py-4 border-b border-ink-700/50 flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1.5">
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${sourceBadgeClass}`}>{sourceLabel}</span>
              {term.bowRank && <span className="text-xs text-fog">#{term.bowRank}</span>}
            </div>
            <h3 className="text-xl font-bold text-white break-all leading-tight" title={term.text}>{term.text}</h3>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg text-mist hover:text-white hover:bg-ink-800/50 transition-colors flex-shrink-0 mt-0.5">
            <CloseIcon />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          {term.bowScore != null && (
            <section>
              <h4 className="text-xs font-semibold text-mist uppercase tracking-wider mb-3 flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-cyan-500 inline-block" />Bolsa de Palabras
              </h4>
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between text-sm mb-1.5">
                    <span className="text-haze">Frecuencia total</span>
                    <span className="font-bold text-cyan-400">{term.bowScore.toLocaleString()}</span>
                  </div>
                  <div className="h-2 rounded-full bg-ink-800/60">
                  ...
                  <p className="text-xs text-fog mt-1">{bowPct.toFixed(1)}% del término más frecuente</p>
                </div>
                {term.bowRank && (
                  <div className="flex justify-between text-sm py-2 border-b border-ink-700/40">
                    <span className="text-mist">Ranking en BoW</span>
                    <span className="text-white font-medium">#{term.bowRank}</span>
                  </div>
                )}
                <div className="flex justify-between text-sm py-2 border-b border-ink-700/40">
                  <span className="text-mist">Corpus analizado</span>
          ...
          <section>
            ...
            {term.tfidfScore != null ? (
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between text-sm mb-1.5">
                    <span className="text-haze">Score TF-IDF</span>
                  ...
                  <p className="text-xs text-fog mt-1">{tfidfPct.toFixed(1)}% del score máximo</p>
                </div>
                {term.idfScore != null && (
                  <div className="flex justify-between text-sm py-2 border-b border-ink-700/40">
                    <span className="text-mist">Valor IDF</span>
                    <span className="text-white font-medium">{term.idfScore.toFixed(2)}</span>
                  </div>
                )}
                {term.tfidfRank && (
                  <div className="flex justify-between text-sm py-2 border-b border-ink-700/40">
                    <span className="text-mist">Ranking TF-IDF</span>
              ...
              <div className="p-3 rounded-lg bg-ink-850/40 border border-ink-700/40">
                <p className="text-xs text-fog text-center">No aparece en el análisis TF-IDF activo.</p>
              </div>
            )}
          </section>
            ...
            {term.relatedNgrams.length > 0 ? (
              <div className="space-y-2">
                {term.relatedNgrams.slice(0, 6).map((ng, i) => (
                  <div key={i} className="flex items-center justify-between gap-3 py-2 border-b border-ink-700/30 last:border-0">
                    <span className="text-sm text-purple-300 font-medium truncate" title={ng.label}>"{ng.label}"</span>
                    <span className="text-xs text-mist flex-shrink-0 bg-ink-850/50 px-2 py-0.5 rounded">{ng.value.toLocaleString()}×</span>
                  </div>
                ))}
                {term.relatedNgrams.length > 6 && (
                  <p className="text-xs text-fog text-center pt-1">+{term.relatedNgrams.length - 6} n-gramas más</p>
                )}
              </div>
            ) : (
              <div className="p-3 rounded-lg bg-ink-850/40 border border-ink-700/40">
                <p className="text-xs text-fog text-center">No encontrado en el top N-gramas activo.</p>
              </div>
            )}
          </section>

          {/* Interpretation */}
          <section className="p-4 rounded-xl bg-gradient-to-br from-ink-850/60 to-ink-900/60 border border-ink-700/40">
            <h4 className="text-xs font-semibold text-mist uppercase tracking-wider mb-2">Interpretación</h4>
