 ...
 *
 * Superficie oscura "tinta" del dashboard público. El acento colorea el icono y,
 * cuando la tarjeta está activa por un filtro cruzado, el borde.
 */
...
// Acentos: los text-X-400 superan 4,5:1 sobre ink-900 (#0D1424).
...
export const ChartCard: React.FC<ChartCardProps> = ({
  ...
  return (
    // Superficie tinta sólida: contraste predecible para gráficos y texto
    <div
      className={`
        group relative overflow-hidden rounded-2xl
        bg-ink-900
        border ${isActive ? colors.border : 'border-ink-700'}
        ${isActive ? `shadow-lg ${colors.glow} ring-1 ${colors.ring}` : ''}
    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-3 px-5 pt-5 pb-3">
        <div className="flex min-w-0 items-start gap-3">
          {icon && (
            // El color del acento queda solo en el icono
            <div className={`mt-0.5 shrink-0 rounded-lg bg-ink-800 p-2 ${colors.text}`}>
              {icon}
            </div>
          )}
          <div className="min-w-0">
            <h3 className="font-display text-[16px] font-semibold leading-snug tracking-[-0.01em] text-paper">{title}</h3>
            {subtitle && (
              <p className="mt-0.5 text-sm leading-snug text-mist">{subtitle}</p>
            )}
          </div>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          {downloadable && (
            <button
              ...
              aria-label="Descargar imagen del gráfico"
              className="p-2 rounded-lg hover:bg-ink-800 transition-opacity opacity-0 group-hover:opacity-100 focus-visible:opacity-100 duration-200"
            >
              {dlState === 'loading' ? (
                <svg className="w-4 h-4 text-mist animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <svg className="w-4 h-4 text-mist hover:text-paper" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {onRefreshClick && (
            ...
            >
              <svg
                className={`w-4 h-4 text-mist hover:text-paper ${isLoading ? 'animate-spin' : ''}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          {onExpandClick && (
            ...
            >
              <svg
                className="w-4 h-4 text-mist hover:text-paper"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
      ...
      {/* Footer */}
      {footer && (
        <div className="relative px-5 py-3 border-t border-ink-700">
          {footer}
        </div>
      )}
    </div>
