 ...
 * AIGeminiPage
 *
 * Página de administración para ejecutar análisis con Gemini (Google).
  ...
  <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-2">
    ...
    {caso.factors.length > 0 && (
      <div className="flex flex-wrap gap-1 pt-1">
        {caso.factors.map((f) => (
          <span key={f} className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200">
...
export const AIGeminiPage: React.FC = () => {
  ...
  const loadHistory = useCallback(async () => {
    try {
      ...
      const data = await aiAnalysisService.getAIConfigsByProvider('gemini');
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (result.success) {
        showSuccess('Análisis con Gemini iniciado correctamente');
        await loadHistory();
      }
    } catch (err: any) {
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-gray-200" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
        <div className="flex items-center justify-between px-4 sm:px-8 py-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <h1 className="text-lg sm:text-xl font-semibold text-gray-900 truncate">Análisis con Gemini</h1>
            <span className="text-xs px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 font-medium shrink-0">Google</span>
          </div>
      ...
      <div className="p-4 sm:p-8 space-y-5 sm:space-y-6">
        ...
        <div className="flex gap-2.5 text-sm text-slate-500 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
          ...
          <span className="leading-relaxed">
            Envía el corpus a <strong>Gemini (Google)</strong> para identificar casos de éxito de transformación digital
        ...
        <div className="bg-white p-5 sm:p-6 space-y-4" style={{ borderRadius: '20px', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
          ...
          <div>
            <label htmlFor="dataset-select-gemini" className="block text-sm font-medium text-slate-700 mb-1.5">
              Dataset a analizar
            </label>
            <select
              id="dataset-select-gemini"
              value={selectedDatasetId}
              onChange={(e) => setSelectedDatasetId(e.target.value ? Number(e.target.value) : '')}
              className="w-full sm:max-w-sm border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent"
            >
          ...
          <div>
            ...
            {showPrompt && (
              <textarea
                ...
                rows={8}
                aria-label="Prompt para Gemini"
                className="mt-2 w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-800 font-mono bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent resize-y"
              />
            )}
          </div>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-full transition-all shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium"
          >
        ...
        <div className="bg-white p-5 sm:p-6" style={{ borderRadius: '20px', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
          ...
          {isLoadingHistory ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-slate-500">Aún no hay análisis ejecutados con Gemini.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {history.map((config) => (
                <div
                  key={config.id}
                  className={`border rounded-xl p-4 transition-colors ${selectedResultId === config.id ? 'border-blue-300 bg-blue-50/50' : 'border-slate-200 bg-white hover:border-slate-300'}`}
                >
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {config.status === 'completed' && (
                        <button
                          ...
                          className="text-xs px-3 py-1 rounded-lg bg-blue-500 hover:bg-blue-600 text-white transition-colors disabled:opacity-50"
                        >
