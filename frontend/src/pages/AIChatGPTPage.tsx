 ...
 * AIChatGPTPage
 *
 * Página de administración para ejecutar análisis con ChatGPT (OpenAI).
 * Misma estructura que AIClaudePage, proveedor distinto.
 */
...
const SuccessCaseCard: React.FC<{ caso: AISuccessCase }> = ({ caso }) => (
  <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-2">
    <div className="flex items-start justify-between gap-2">
      ...
      <span className="text-xs text-slate-400 shrink-0">{(caso.confidence * 100).toFixed(0)}% confianza</span>
    </div>
    <p className="text-xs text-slate-500"><span className="font-medium text-slate-700">Tipo:</span> {caso.transformation_type}</p>
    <p className="text-xs text-slate-600 leading-relaxed">{caso.results}</p>
    {caso.factors.length > 0 && (
      <div className="flex flex-wrap gap-1 pt-1">
        {caso.factors.map((f) => (
          <span key={f} className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">
...
export const AIChatGPTPage: React.FC = () => {
  ...
  const loadHistory = useCallback(async () => {
    try {
      ...
      const data = await aiAnalysisService.getAIConfigsByProvider('chatgpt');
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (result.success) {
        showSuccess('Análisis con ChatGPT iniciado correctamente');
        await loadHistory();
      }
    } catch (err: any) {
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-gray-200" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
        <div className="flex items-center justify-between px-4 sm:px-8 py-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <h1 className="text-lg sm:text-xl font-semibold text-gray-900 truncate">Análisis con ChatGPT</h1>
            <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 font-medium shrink-0">OpenAI</span>
          </div>
          <button onClick={loadHistory} className="p-2 sm:p-2.5 bg-gray-100 rounded-full hover:bg-gray-200 transition-colors" title="Actualizar historial">
      ...
      <div className="p-4 sm:p-8 space-y-5 sm:space-y-6">
        ...
        <div className="flex gap-2.5 text-sm text-slate-500 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
          ...
          <span className="leading-relaxed">
            Envía el corpus a <strong>ChatGPT (OpenAI)</strong> para identificar casos de éxito de transformación digital
        ...
        <div className="bg-white p-5 sm:p-6 space-y-4" style={{ borderRadius: '20px', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
          ...
          <div>
            <label htmlFor="dataset-select-chatgpt" className="block text-sm font-medium text-slate-700 mb-1.5">
              Dataset a analizar
            </label>
            <select
              id="dataset-select-chatgpt"
              value={selectedDatasetId}
              onChange={(e) => setSelectedDatasetId(e.target.value ? Number(e.target.value) : '')}
              className="w-full sm:max-w-sm border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent"
            >
          ...
          <div>
            ...
            {showPrompt && (
              <textarea
                ...
                className="mt-2 w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-800 font-mono bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent resize-y"
              />
            )}
          </div>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-full transition-all shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium"
          >
        ...
        <div className="bg-white p-5 sm:p-6" style={{ borderRadius: '20px', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
          ...
          {isLoadingHistory ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-slate-500">Aún no hay análisis ejecutados con ChatGPT.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {history.map((config) => (
                <div
                  key={config.id}
                  className={`border rounded-xl p-4 transition-colors ${selectedResultId === config.id ? 'border-emerald-300 bg-emerald-50/50' : 'border-slate-200 bg-white hover:border-slate-300'}`}
                >
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-slate-800 truncate">Dataset: {config.dataset_name}</p>
                      <p className="text-xs text-slate-400 mt-0.5">{new Date(config.created_at).toLocaleString('es-CO')}</p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {config.status === 'completed' && (
                        <button
                          ...
                          className="text-xs px-3 py-1 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white transition-colors disabled:opacity-50"
                        >
                          {isLoadingResult && selectedResultId === config.id ? <Spinner /> : 'Ver resultados'}
                        </button>
                      )}
                    </div>
                  </div>
                  {config.status === 'error' && config.error_message && (
                    <p className="text-xs text-red-600 mt-2 bg-red-50 rounded-lg px-3 py-2">{config.error_message}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        {selectedResult && (
          <div className="bg-white p-5 sm:p-6 space-y-5" style={{ borderRadius: '20px', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div className="flex items-start justify-between flex-wrap gap-2">
              ...
              <div className="flex items-center gap-3">
                {selectedResult.processing_time_seconds != null && (
                  <span className="text-xs text-slate-400">Tiempo: {selectedResult.processing_time_seconds.toFixed(1)}s</span>
                )}
            ...
            {selectedResult.factors_found.length > 0 && (
              <div>
                <p className="text-sm font-medium text-slate-700 mb-2">Factores identificados ({selectedResult.factors_found.length})</p>
                <div className="flex flex-wrap gap-2">
                  {selectedResult.factors_found.map((f) => (
                    <span key={f} className="text-xs px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">{f}</span>
                  ))}
                </div>
              </div>
            )}
            {selectedResult.success_cases.length > 0 && (
              <div>
                <p className="text-sm font-medium text-slate-700 mb-3">Casos de éxito ({selectedResult.success_cases.length})</p>
