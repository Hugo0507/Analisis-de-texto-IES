/**
 * TfIdfList Page - Lista de Análisis TF-IDF
 *
 * Vista principal del módulo de Matriz TF-IDF.
...
import type { TfIdfAnalysisListItem } from '../services/tfidfAnalysisService';
import { Spinner } from '../components/atoms';
import { useToast } from '../contexts/ToastContext';

export const TfIdfList: React.FC = () => {
  const navigate = useNavigate();
  const { showSuccess, showError } = useToast();
  const [analyses, setAnalyses] = useState<TfIdfAnalysisListItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [analysisToDelete, setAnalysisToDelete] = useState<TfIdfAnalysisListItem | null>(null);
  ...
  const loadAnalyses = async () => {
    setIsLoading(true);
    try {
      const data = await tfidfAnalysisService.getTfIdfAnalyses();
      setAnalyses(data);
    } catch (error: any) {
  ...
  const openDeleteModal = (analysis: TfIdfAnalysisListItem) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await tfidfAnalysisService.deleteTfIdfAnalysis(analysisToDelete.id);
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-gray-200" style={{ boxShadow: '0 1px 3px rgba(0, 0, 0, 0.02)' }}>
        <div className="flex items-center justify-between px-8 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-semibold text-gray-900">Matriz TF-IDF</h1>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Add Button */}
            <button
              onClick={() => navigate('/admin/vectorizacion/tf-idf/nuevo')}
          ...
          {analyses.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-gray-600 mb-6">
                Comienza creando tu primer análisis TF-IDF
              </p>
              <button
                onClick={() => navigate('/admin/vectorizacion/tf-idf/nuevo')}
                ...
                <tbody>
                  {analyses.map((analysis, index) => (
                    <tr
                      ...
                      onClick={() => navigate(`/admin/vectorizacion/tf-idf/${analysis.id}`)}
                    >
                      {/* ID Column */}
                      ...
                      <td className="px-6 py-4">
                        <div>
                          <p className="text-sm font-medium text-gray-900">{analysis.source_type_label}</p>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-600">
                          {analysis.vocabulary_size > 0 ? analysis.vocabulary_size.toLocaleString() : '-'}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-600">
                          {analysis.document_count > 0 ? analysis.document_count.toLocaleString() : '-'}
                        </span>
                      </td>
