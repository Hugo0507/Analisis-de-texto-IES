/**
 * NgramsList Page - Lista de Análisis de N-gramas
 *
 * Vista principal del módulo de Análisis de N-gramas.
...
export const NgramsList: React.FC = () => {
  const navigate = useNavigate();
  const { showSuccess, showError } = useToast();
  const [analyses, setAnalyses] = useState<NgramAnalysisListItem[]>([]);
  ...
  const loadAnalyses = async () => {
    setIsLoading(true);
    try {
      const data = await ngramAnalysisService.getNgramAnalyses();
      setAnalyses(data);
    } catch (error: any) {
  ...
  const openDeleteModal = (analysis: NgramAnalysisListItem) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await ngramAnalysisService.deleteNgramAnalysis(analysisToDelete.id);
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-gray-200" style={{ boxShadow: '0 1px 3px rgba(0, 0, 0, 0.02)' }}>
        <div className="flex items-center justify-between px-8 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-semibold text-gray-900">Análisis de N-gramas</h1>
          </div>
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-600">
                          {analysis.document_count > 0 ? analysis.document_count : '-'}
                        </span>
                      </td>
