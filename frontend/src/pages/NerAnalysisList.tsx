/**
 * NerAnalysisList Page - Lista de Análisis NER
 *
 * Vista principal del módulo de Reconocimiento de Entidades Nombradas.
...
import type { NerAnalysisListItem } from '../services/nerAnalysisService';
import { Spinner } from '../components/atoms';
import { useToast } from '../contexts/ToastContext';

export const NerAnalysisList: React.FC = () => {
  const navigate = useNavigate();
  const { showSuccess, showError } = useToast();
  const [analyses, setAnalyses] = useState<NerAnalysisListItem[]>([]);
  ...
  const loadAnalyses = async () => {
    setIsLoading(true);
    try {
      const data = await nerAnalysisService.getNerAnalyses();
      setAnalyses(data);
    } catch (error: any) {
  ...
  const openDeleteModal = (analysis: NerAnalysisListItem) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await nerAnalysisService.deleteNerAnalysis(analysisToDelete.id);
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-gray-200" style={{ boxShadow: '0 1px 3px rgba(0, 0, 0, 0.02)' }}>
        <div className="flex items-center justify-between px-8 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-semibold text-gray-900">NER - Entidades Nombradas</h1>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Add Button */}
            <button
              onClick={() => navigate('/admin/modelado/ner/nuevo')}
          ...
          {analyses.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-gray-600 mb-6">
                Comienza creando tu primer análisis de Reconocimiento de Entidades Nombradas
              </p>
              <button
                onClick={() => navigate('/admin/modelado/ner/nuevo')}
                ...
                <tbody>
                  {analyses.map((analysis, index) => (
                    <tr
                      ...
                      onClick={() => navigate(`/admin/modelado/ner/${analysis.id}`)}
                    >
                      {/* ID Column */}
                      ...
                      {/* Modelo Column */}
                      <td className="px-6 py-4">
                        <span className="text-sm text-gray-600">{analysis.spacy_model_label}</span>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-600">
                          {analysis.documents_processed > 0 ? `${analysis.documents_processed} docs` : '-'}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm">
                          <p className="font-medium text-gray-900">
                            {analysis.total_entities_found > 0 ? analysis.total_entities_found.toLocaleString() : '-'}
                          </p>
                          {analysis.unique_entities_count > 0 && (
                            <p className="text-xs text-gray-500">
                              {analysis.unique_entities_count} únicas
                            </p>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                          ...
                          <button
                            ...
                            className="p-2 text-gray-500 hover:text-red-600 rounded-lg transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:ring-offset-1"
          ...
          <div className="bg-white p-6 max-w-md w-full mx-4" style={{ borderRadius: '20px', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.1)' }}>
            ...
            <div className="flex items-center justify-end gap-3">
              <button
                ...
                className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors font-medium text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-gray-400"
              >
                Cancelar
              </button>
              <button
                onClick={confirmDelete}
                className="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg transition-colors font-medium text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-red-400"
              >
